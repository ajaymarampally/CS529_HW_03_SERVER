...
const ParticleSystem = function() {
    ...
    var RdYlBuScale = d3.scaleSequential( d3.interpolateRdYlBu ).domain([0, 36]);
    var greyScale = d3.scaleSequential( d3.interpolateGreys ).domain([0, 36]);

    var PlaneGeometry = new THREE.PlaneGeometry( 12,12 );
    var PlaneMaterial = new THREE.MeshBasicMaterial( {color: 0xffffff, side: THREE.DoubleSide, transparent:true, opacity:0.4 } );
    ...
    self.createParticleSystem = function() {
        ...
        for(var k = 0 ; k < data.length ; k++ ){
            ...
            col.set( RdYlBuScale( data[k].concentration ) );
        ...
        var particlematerial = new THREE.PointsMaterial( { vertexColors: THREE.VertexColors, opacity:0.6, transparent:true, size:0.06 } );
    ...
    self.section = function(z) {
        ...
        section.selectAll('circle')
            ...
            .style('fill', function(d) { return RdYlBuScale( d.concentration ); } );
    };
    ...
    $( "#zSlider" ).slider({value: 0,min: -5,max: 5,step: 0.01,
    ...
    $(document).keydown(function(e) {
        switch(e.which) {
            case 37: // left
            sceneObject.rotation.y += 0.1;

            break;

            case 38: // up
            sceneObject.rotation.x += 0.1;
            break;
